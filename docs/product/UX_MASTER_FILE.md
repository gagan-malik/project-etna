...
> **Author:** Gagan Malik  
> **Status:** Baseline Definition (Unauthenticated, Voice, 4-Mode, BYOK, MCP)
...
5. [AI Model Selection & BYOK](#ai-model-selection--byok)
6. [MCP & Tool Calling](#mcp--tool-calling)
7. [User Personas](#user-personas)
8. [Information Architecture](#information-architecture)
9. [Navigation Model](#navigation-model)
10. [Task Flows](#task-flows)
11. [End-to-End User Journeys](#end-to-end-user-journeys)
...
16. [UX Metrics & Success Criteria](#ux-metrics--success-criteria)
...
## AI Model Selection & BYOK

### Overview

Etna supports multiple AI models with BYOK (Bring Your Own Key) as a Pro feature.
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  FREE TIER                         PRO TIER (with BYOK)            │
│  ──────────                        ─────────────────────            │
│                                                                     │
│  ┌─────────────┐                   ┌─────────────────────────────┐ │
│  │ DeepSeek V3 │                   │  Choose Your Model          │ │
│  │  (Default)  │                   │  ───────────────────────    │ │
│  │             │                   │  • DeepSeek V3 (default)    │ │
│  │ 50 queries  │                   │  • GPT-4o, GPT-4            │ │
│  │    /day     │                   │  • Claude 3.5/4 Sonnet      │ │
│  └─────────────┘                   │  • Llama 3.3, Mistral       │ │
│                                    │  • 400+ via OpenRouter      │ │
│                                    │                             │ │
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  ⚙️ SETTINGS > AI Models                                  [Pro ✓]  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  DEFAULT MODEL                                                      │
│  ─────────────                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ DeepSeek V3 (Recommended - Best price/performance)      ▼   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  BRING YOUR OWN KEY (BYOK)                              [Pro Only] │
│  ─────────────────────────────────────────────────────────────────  │
│  Add your API keys to unlock unlimited queries and more models.    │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 🟢 OpenRouter    sk-or-v1-••••••••••••   ✓ Connected       │   │
│  │    💡 Gives access to 400+ models with one key             │   │
│  │    [Test Connection]  [Remove]                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ⚪ OpenAI        [Add API Key]                              │   │
│  │    Unlocks: GPT-4o, GPT-4, O1, O3                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ⚪ Anthropic     [Add API Key]                              │   │
│  │    Unlocks: Claude 3.5 Sonnet, Claude 4                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ⚪ Google AI     [Add API Key]                              │   │
│  │    Unlocks: Gemini Pro, Gemini Ultra                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ─────────────────────────────────────────────────────────────────  │
│                                                                     │
...
│  │─────────│──────────────────│───────────────────────────────│   │
│  │ Ask     │ DeepSeek V3      │ Cheap, good for Q&A           │   │
│  │ Agent   │ Claude 3.5       │ Best at complex instructions  │   │
│  │ Debug   │ DeepSeek V3      │ Strong reasoning              │   │
│  │ Manual  │ Any              │ Simple edits, doesn't matter  │   │
│                                                                     │
│  [✓] Auto-select recommended model per mode                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### Free Tier Upsell (BYOK Gate)
...
│  │                                                             │   │
│  │ Upgrade to Pro to:                                         │   │
│  │ • Use your own API keys (unlimited queries)                │   │
│  │ • Access GPT-4, Claude, and 400+ models                    │   │
...
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Create a testbench for this FIFO...      DeepSeek V3 ▼  🎤 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                              │                      │
│                                              ▼                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
...
│  │ All Models                                                  │   │
│  │ ──────────                                                  │   │
│  │ ○ DeepSeek V3          Fast & cheap (default)              │   │
│  │ ○ Llama 3.3 70B        Open source, fast                   │   │
│  │ ○ Mistral Large        Strong on code                      │   │
│  │ ○ Gemini Pro           Good for analysis                   │   │
│  │                                                             │   │
│  │ [Browse all 400+ models...]                                │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
Etna uses MCP (Model Context Protocol) for its tool architecture - the emerging industry standard for AI-tool integration backed by Anthropic and OpenAI.

```
┌─────────────────────────────────────────────────────────────────────┐
│                     ETNA TOOL ARCHITECTURE                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  USER QUERY                                                         │
│      │                                                              │
...
│  │  • User intent                                               │   │
│  │  • Current mode (Ask/Agent/Debug/Manual)                    │   │
│  │  • Available context                                         │   │
│  └─────────────────────────────────────────────────────────────┘   │
│      │                                                              │
│      ▼                                                              │
...
│      ▼              ▼              ▼              ▼                │
│  ┌────────┐    ┌────────┐    ┌────────┐    ┌────────┐             │
│  │ Surfer │    │sv-parser│   │  Vercel│    │Verilator│            │
│  │  WASM  │    │tree-sitter│ │  Blob  │    │  WASM  │             │
│  └────────┘    └────────┘    └────────┘    └────────┘             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### Silicon-Specific Tools (Etna Differentiators)

These AI-powered tools don't exist in any competitor:

```
┌─────────────────────────────────────────────────────────────────────┐
│                 ETNA-EXCLUSIVE MCP TOOLS                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  WAVEFORM TOOLS (etna-waveform MCP)                                │
│  ─────────────────────────────────                                  │
│  • analyze_waveform    - Parse VCD/FST, find transitions           │
│  • find_signal_edge    - "Find when clk_en goes high"              │
│  • measure_timing      - Setup/hold, period measurements           │
│  • detect_protocol_violation - AXI/APB handshake checking          │
│  • compare_signals     - Golden vs actual comparison               │
│                                                                     │
│  RTL TOOLS (etna-rtl MCP)                                          │
│  ────────────────────────                                           │
│  • parse_rtl           - Extract module hierarchy, ports           │
│  • trace_signal_source - Find where a signal is driven             │
│  • find_clock_domain   - Identify CDC boundaries                   │
│  • generate_assertion  - Create SVA from observed behavior         │
│  • analyze_fsm         - Extract and visualize state machines      │
│                                                                     │
│  SIMULATION TOOLS (etna-sim MCP)                                   │
│  ───────────────────────────────                                    │
│  • run_simulation      - Execute Verilator WASM in browser         │
│  • capture_waveform    - Generate VCD from simulation              │
│  • check_assertions    - Run SVA checks                            │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  You: Why is data_valid stuck low after the 5th transaction?      │
│                                                                     │
...
│  │ 🔧 Using tools:                                             │   │
│  │                                                             │   │
│  │ ├─ analyze_waveform("data_valid", time_range: 0-1000ns)    │   │
│  │ │  └─ Found: Goes high at 100ns, 200ns, 300ns, 400ns, 500ns│   │
│  │ │     Then stays low after 500ns                           │   │
│  │ │                                                           │   │
│  │ ├─ find_signal_edge("fifo_full", edge: "rising")           │   │
│  │ │  └─ Found: Rising edge at 495ns                          │   │
│  │ │                                                           │   │
│  │ └─ trace_signal_source("data_valid")                       │   │
│  │    └─ Driven by: fifo_ctrl.sv:47 (gated by ~fifo_full)    │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  **Root Cause Found:**                                             │
│                                                                     │
│  `data_valid` is gated by `fifo_full` in fifo_ctrl.sv line 47.    │
│  The FIFO becomes full at 495ns (5th transaction), which blocks   │
│  `data_valid` from asserting.                                      │
│                                                                     │
│  The FIFO depth is set to 5 - is this intentional, or should it  │
│  be larger?                                                        │
│                                                                     │
│  [Show in Waveform]  [Go to Code]  [Increase FIFO Depth]          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
│  │ 🔄 Running tools...                                         │   │
│  │                                                             │   │
│  │ ✅ parse_rtl("top.sv")                           0.2s      │   │
│  │ ✅ analyze_waveform("clk")                       0.5s      │   │
│  │ 🔄 run_simulation("tb_top.sv")                   running   │   │
│  │    ━━━━━━━━━━━━━━━━━░░░░░░░░░░░░░░░ 45%                   │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
...
### Simulation: WASM-First Strategy

Etna runs simulations in the browser using Verilator compiled to WebAssembly:
...
│                    • ~10K lines RTL limit                          │
│                                                                     │
│  FUTURE: Server-Side (Enterprise)                                  │
│  ─────────────────────────────────                                  │
│  • Large designs (100K+ lines)                                     │
│  • Commercial simulators (VCS, Questa)                             │
│  • Longer simulations                                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### MCP Server Settings (Future)

```
┌─────────────────────────────────────────────────────────────────────┐
│  ⚙️ SETTINGS > Tools & Integrations                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  BUILT-IN TOOLS                                                    │
│  ─────────────                                                      │
│  [✓] Waveform Analysis (etna-waveform)                             │
│  [✓] RTL Parsing (etna-rtl)                                        │
│  [✓] Simulation (etna-sim) - Verilator WASM                        │
│  [✓] File System                                                   │
│                                                                     │
│  EXTERNAL INTEGRATIONS                               [Pro Feature] │
│  ─────────────────────                                              │
│  [ ] GitHub - Connect repositories                   [Connect]     │
│  [ ] Jira - Link bugs to debug sessions             [Connect]     │
│  [ ] Confluence - Access design specs               [Connect]     │
│                                                                     │
│  CUSTOM MCP SERVERS                                  [Enterprise]  │
│  ───────────────────                                                │
│  Add your own MCP servers for custom tool integrations.            │
│  [+ Add MCP Server]                                                │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| January 2026 | 1.5 | Added BYOK (Bring Your Own Key) & multi-model support |
| January 2026 | 1.6 | Added MCP & tool calling architecture (waveform, RTL, simulation) |
