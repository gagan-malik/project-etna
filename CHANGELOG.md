...
### Added
- **Agent Orchestration Layer** – Runtime orchestration for multi-agent pipelines: intent classification (rule-based), routing to Enterprise/UX agents, execution pipeline with persistence. API: `POST /api/orchestration/run`, `POST /api/orchestration/run/stream`, `GET /api/orchestration/runs`, `GET /api/orchestration/runs/[id]`. Chat UI: orchestration mode toggle, `/orchestrate` slash command, run progress display, SSE consumer. Activity page: Orchestration Runs tab; run detail at `/orchestration/runs/[id]`. Conversation message appended on completion. See [docs/spikes/agent-orchestration-spike.md](docs/spikes/agent-orchestration-spike.md) and [docs/api/orchestration.md](docs/api/orchestration.md).
